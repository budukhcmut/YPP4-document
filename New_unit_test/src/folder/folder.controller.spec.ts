...
controller = module.get<FolderController>(FolderController);
  ...
  describe('getMyFolders', () => {
    it('should return folders for a specific user', async () => {
      ...
      const result = await controller.getMyFolders(userId);
  ...
  describe('getMyFoldersFull', () => {
    it('should return full folder details for a specific user', async () => {
      const userId = 1; // Assuming this user exists
      const result = await controller.getMyFoldersFull(userId);
      expect(result).toBeDefined();
      expect(Array.isArray(result)).toBe(true);
    });
